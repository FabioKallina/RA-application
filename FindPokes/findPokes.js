...
const answer2Or = pokes.filter(poke => types.some(type => poke.types.includes(type))).map(poke => poke.name); // TODO
const answer2And = pokes.filter(poke => types.every(type => poke.types.includes(type))).map(poke => poke.name); // TODO
